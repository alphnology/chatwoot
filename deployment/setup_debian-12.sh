...
function install_dependencies() {
  ...
  echo "deb [signed-by=/usr/share/keyrings/postgresql-keyring.gpg] http://apt.postgresql.org/pub/repos/apt/ bookworm-pgdg main" | tee /etc/apt/sources.list.d/pgdg.list
  ...
  apt-get install -y \
      ...
      libgmp-dev libncurses5-dev libffi-dev libgdbm-dev sudo \
      libvips python3-pip wget
  npm install -g pnpm
}
...
function setup_chatwoot() {
  ...
  git clone https://github.com/alphnology/chatwoot.git
  cd chatwoot
  git checkout "$BRANCH"
  bundle
  pnpm i
...
function configure_systemd_services() {
  ...
  cp /home/chatwoot/chatwoot/deployment/setup_debian-12.sh /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/alphnology/chatwoot/develop/deployment/nginx_chatwoot.conf
...
function report_event() {
  ...
  echo "{\"installation_identifier\":\"$installation_identifier\",\"event_name\":\"$event_name\",\"event_data\":{\"action\":\"$event_data\"}}"
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/alphnology/chatwoot/master/VERSION_CWCTL"
...
function install_packaging() {
  # Debian 12 uses PEP 668, so we need to use apt instead of pip
  echo "Detected Debian 12. Installing packaging library using apt."
